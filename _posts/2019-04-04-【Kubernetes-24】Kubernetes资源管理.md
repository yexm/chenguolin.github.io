...
1. requests.cpu: 字段设置的是 cpu.shares 的值，值计算公式为 **requests.cpu * 1024**。例如 requests.cpu 值为100m 转换为小数就是 0.1，则cpu.shares 的值为102（取整）。cpu.shares 值表示进程使用CPU的权重设置，默认值为1024，最小值为2。
