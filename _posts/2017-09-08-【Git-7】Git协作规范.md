...
5. `release`: 版本迭代发布分支，下一个功能版本要上线之前基于develop开的一个分支，新版本上线的时候合并到master进行上线。（没有develop就用master）
6. `feature`: 独立新功能开发分支，基于develop（没有develop就基于master）开的一个分支，开发完成合并到develop分支进行测试。（没有develop就用master）
